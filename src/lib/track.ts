...
function sumTrackSegment(trackPoints: TrackPointWithMetadata[]): TrackSegment {
    ...
    const totalDistance = trackPoints.reduce((sum, tp) => sum + (tp.distanceFromLastPoint || 0), 0);
    const startTime = trackPoints[0].time;
    const endTime = trackPoints[trackPoints.length - 1].time;
    const averageHeartRate = trackPoints.reduce((sum, tp) => sum + (tp.hr || 0), 0) / trackPoints.length;
    const averageCadence = trackPoints.reduce((sum, tp) => sum + (tp.cad || 0), 0) / trackPoints.length;
    const averagePower = trackPoints.reduce((sum, tp) => sum + (tp.power || 0), 0) / trackPoints.length;

    return {
        distanceAtStart: trackPoints[0].distanceTraveledUntilThisPoint,
        distanceAtEnd: trackPoints[trackPoints.length - 1].distanceTraveledUntilThisPoint,
...
function calculateAveragePace(trackPoints: TrackPointWithMetadata[]): string {
    ...
    for (const point of trackPoints) {
        if (previousPoint) {
            ...
            totalTimeInMinutes += (point.time.getTime() - previousPoint.time.getTime()) / 60000; // convert milliseconds to minutes
        }
        previousPoint = point;
    }
    ...
    const averagePaceInMinutesPerKm = totalTimeInMinutes / (totalDistanceInMeters / 1000);
...
function calculateTotalDistance(trackPoints: TrackPointWithMetadata[]): number {
    return trackPoints.reduce((total, tp) => total + (tp.distanceFromLastPoint || 0), 0);
}

function calculateTrackPointMetadata(trackPoints: TrackPointWithMetadata[]): void {
    trackPoints.reduce((prev, next) => {
        ...
        next.distanceTraveledUntilThisPoint = (prev.distanceTraveledUntilThisPoint || 0) + distanceTraveledInMeters;
        ...
        const paceInMinutesPerKm = timeInMinutes / (distanceTraveledInMeters / 1000); // minutes per kilometer
